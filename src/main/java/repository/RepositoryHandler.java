        ...
        try {
            ...
            //issues=this.getIterateIssues();
                ...
                // il path non esite: è un url remoto?
        			try {
						this.remoteURL = DbHandler.getURLByProjectPath(URL);
						System.out.println("THE REMOTE URL IS = " + this.remoteURL);
						if(this.remoteURL == null) {
							this.remoteURL = URL;
							System.out.println("THE REMOTE URL IS = "+URL);
			                this.localPath = Files.createTempDirectory("SBAC-" + this.repoNameFromURI());
			                cloneGitRepository();
						} else {
							this.localPath = Files.createTempDirectory("SBAC-" + this.repoNameFromURI());
		                    repository = this.entryGitApi();
		                    cloneGitRepository();
						}
