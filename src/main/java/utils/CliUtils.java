...
public class CliUtils {
    ...
    public CliUtils(String command, String ... args ) {
        ...
        if (command.equals("PhDSmells")||command.equals("CodeShovel")) {
    ...
    public Result execute() throws Exception,IOException {
        ...
        //The field COMMAND contains the name of the tool ONLY for PhDProjectScripts or CodeShovel
        if (command.equals("PhDSmells")||command.equals("CodeShovel")){

            ProcessBuilder promptBuilder = new ProcessBuilder(args);
            //System.out.println(command + " ProcessBuilder = " + promptBuilder);
            promptBuilder.redirectErrorStream(true);

            Process prompt = promptBuilder.start();
            //System.out.println(command + " ProcessBuilder started");
            ...
            process.destroy();
            if(command == "java")
                System.out.println("SpotBugs = " + exitCode);
            else
                System.out.println("Sourcemeter = " + exitCode);
            return new Result(exitCode, output);
